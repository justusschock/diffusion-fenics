...
namespace ConvectionDiffusion {
    ...
    convectionDiffusion1D::FunctionSpace
        {return convectionDiffusion1D::FunctionSpace(mesh);}

        auto LinearForm(std::shared_ptr<convectionDiffusion1D::FunctionSpace>
    FunctionSpace) -> convectionDiffusion1D::LinearForm
        { return convectionDiffusion1D::LinearForm(FunctionSpace);}

        auto BilinearForm(std::shared_ptr<convectionDiffusion1D::FunctionSpace>
    FunctionSpace1,
                          std::shared_ptr<convectionDiffusion1D::FunctionSpace>
    FunctionSpace2) -> convectionDiffusion1D::BilinearForm
        {return convectionDiffusion1D::BilinearForm(FunctionSpace1,
    FunctionSpace2);}

        auto VelocityFunctionSpace(std::shared_ptr<dolfin::Mesh> mesh) ->
    velocity1D::FunctionSpace
    ...
    public:
        auto FunctionSpace(std::shared_ptr<dolfin::Mesh> mesh) ->
    convectionDiffusion2D::FunctionSpace
        {return convectionDiffusion2D::FunctionSpace(mesh);}

        auto LinearForm(std::shared_ptr<convectionDiffusion2D::FunctionSpace>
    FunctionSpace) -> convectionDiffusion2D::LinearForm
        { return convectionDiffusion2D::LinearForm(FunctionSpace);}

        auto BilinearForm(std::shared_ptr<convectionDiffusion2D::FunctionSpace>
    FunctionSpace1,
                          std::shared_ptr<convectionDiffusion2D::FunctionSpace>
    FunctionSpace2) -> convectionDiffusion2D::BilinearForm
        {return convectionDiffusion2D::BilinearForm(FunctionSpace2,
    FunctionSpace2);}

        auto VelocityFunctionSpace(std::shared_ptr<dolfin::Mesh> mesh) ->
    velocity2D::FunctionSpace
    ...
    // Provide FunctionSpace, Linear and Bilinear Form in 2D
    template <>
    class DimensionWrapper<3> {
       public:
        auto FunctionSpace(std::shared_ptr<dolfin::Mesh> mesh)
            -> convectionDiffusion3D::FunctionSpace
        {
            return convectionDiffusion3D::FunctionSpace(mesh);
        }

        auto LinearForm(
            std::shared_ptr<convectionDiffusion3D::FunctionSpace> FunctionSpace)
            -> convectionDiffusion3D::LinearForm
        {
            return convectionDiffusion3D::LinearForm(FunctionSpace);
        }

        auto BilinearForm(std::shared_ptr<convectionDiffusion3D::FunctionSpace>
                              FunctionSpace1,
                          std::shared_ptr<convectionDiffusion3D::FunctionSpace>
                              FunctionSpace2)
            -> convectionDiffusion3D::BilinearForm
        {
            return convectionDiffusion3D::BilinearForm(FunctionSpace1,
                                                       FunctionSpace2);
        }

        auto VelocityFunctionSpace(std::shared_ptr<dolfin::Mesh> mesh)
            -> velocity3D::FunctionSpace
        {
